...
Version **3.3** secures saved configuration and local playback, prevents false or lost monitoring alerts and makes file-based runtime state fail visibly instead of remaining stale. The project itself gains a published security policy with private vulnerability reporting, guided issue and pull request templates, contribution and dependency licensing documentation, continuous supply chain scanning that audits dependencies, publishes an SBOM and scans the container image, plus a PyPI release that cannot publish until the full test suite passes. Release downloads are now verifiable, the repository can be cited directly from its GitHub page and an automated defect check runs on every change.
...
- **IMPROVE:** **Pinned and gated release automation** - Every workflow now pins third-party GitHub Actions to a specific commit with the released version recorded alongside it, so a moved tag cannot change what runs in the jobs that hold PyPI, Docker Hub and GitHub release credentials. **Publishing to PyPI now runs the full test suite first and stops if it fails**, matching the Docker Hub release, so no untested build is ever released under the project's name. Release tags and workflow inputs reach build scripts through the environment and are checked against the characters an image tag may contain, instead of being pasted into a shell command. Dependabot keeps the pins current and new contract tests fail the build if any action returns to a mutable tag or if the version reported by `--version`, the package metadata and these release notes ever disagree
...
- **IMPROVE:** **Published security and contribution policies** - A **security policy** documents private vulnerability reporting through GitHub advisories, the supported versions and what a report must never contain. New **issue forms** route suspected vulnerabilities away from public issues and collect the version, install method, monitoring mode and Doctor output up front, while **contribution guidance**, a **code of conduct** and a **third-party dependency licensing notice** describe the development setup, the checks CI enforces and the license of every declared dependency. A new **test suite guide** maps every test file to the area it covers and states the offline and credential-safety rules a contributed test must follow
- **IMPROVE:** **Verifiable release downloads** - Each published release now attaches a **`SHA256SUMS.txt`** file next to the `zip` and `tar.gz` archives, and both archives carry a signed build provenance attestation. You can confirm a download really came from this repository before you unpack it with `gh attestation verify spotify_monitor_<tag>.zip --repo misiektoja/spotify_monitor`
- **IMPROVE:** **Automated defect checks on every change** - A pinned [Ruff](https://docs.astral.sh/ruff/) lint pass now runs in CI before the test suite, reporting unused names, undefined names and common bug patterns. Added optional pre-commit hooks and a shared [.editorconfig](https://github.com/misiektoja/spotify_monitor/blob/main/.editorconfig) that records the project's existing style
- **IMPROVE:** **Cite the project** - The repository page now offers **Cite this repository**, which exports a ready-made BibTeX or APA entry from the new [CITATION.cff](https://github.com/misiektoja/spotify_monitor/blob/main/CITATION.cff). The new [SUPPORT.md](https://github.com/misiektoja/spotify_monitor/blob/main/SUPPORT.md) shows where a question, a bug report and a vulnerability each belong and what to include, starting with **`--doctor`** output

**Bug fixes**:

- **BUGFIX:** **Hardened email notifications** - The monitored friend's Spotify display name is now HTML-escaped everywhere it appears in an email body, so a crafted display name can no longer inject markup, a live link or a tracking image into a message you trust. Every link address in an HTML email is attribute-escaped as well, matching the link text that already was. A new check reads the source itself, so a future notification that forgets to escape Spotify-supplied text fails the build
- **BUGFIX:** **Connectivity check respects configuration** - The startup internet check now honors `CHECK_INTERNET_URL`, `CHECK_INTERNET_TIMEOUT` and `VERIFY_SSL` from the config file or dotenv instead of the built-in defaults frozen when the tool loads. `VERIFY_SSL = False` also suppresses insecure-request warnings as intended, so a TLS-inspecting proxy setup is no longer blocked at startup by settings you already changed
- **BUGFIX:** **Reliable Spotify links** - Links built from a Spotify URI are now parsed by exact parts, so a malformed reference produces no link at all instead of a wrong one, and an identifier that merely contains another object type can no longer redirect the link
...
- **BUGFIX:** **Declarative configuration and private backups** - Configuration files now accept only documented literal assignments and reject imports, calls, expressions or control flow without execution, so a configuration file sitting in the working directory can no longer run code. The rejected line and setting are named on startup and by `--doctor`. **Configuration files carried across upgrades keep working:** settings that an older version wrote and a newer one removed, such as `SECRET_CIPHER_DICT`, `SECRET_CIPHER_DICT_URL` and `TOTP_VER` from versions 2.3.1 through 2.9.2, are ignored with a note instead of stopping startup, while a misspelled setting is still reported. Generated backups retain owner-only permissions and effective timing or count values are validated before network activity
