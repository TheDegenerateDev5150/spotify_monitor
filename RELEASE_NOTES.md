...
- **BUGFIX:** **Declarative configuration and private backups** - Configuration files now accept only documented literal assignments and reject imports, calls, expressions or control flow without execution, so a configuration file sitting in the working directory can no longer run code. The rejected line and setting are named on startup and by `--doctor`. **Upgrading from version 2.x may need one edit:** configuration files generated by versions 2.3.1 through 2.9.2 contain `SECRET_CIPHER_DICT`, `SECRET_CIPHER_DICT_URL` or `TOTP_VER`, which version 3.0 removed and which were silently ignored until now. Delete those lines, or run `--generate-config` for a fresh file. Generated backups retain owner-only permissions and effective timing or count values are validated before network activity
