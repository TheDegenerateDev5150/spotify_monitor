...
Version **3.3** focuses on trustworthy alerts, a leaner install and safer configuration. Notification emails can no longer be tampered with through a monitored friend's display name, delivery failures are retried instead of silently lost, false Last.fm outage alerts are gone and a configuration file can no longer run code. Local playback control, connectivity checks and Spotify links were all hardened, and access tokens are reused until they expire, roughly halving Friend Activity requests to Spotify. Artwork in ntfy alerts became an optional extra so a standard install carries one less image library. Release downloads can now be verified before you unpack them, and the project gained a published security policy with private vulnerability reporting plus automated security and supply chain scanning.

**Features and improvements**:

- **IMPROVE:** **Leaner install with optional ntfy artwork** - **Pillow is no longer a required dependency**, so a standard install pulls one less image library and no longer inherits its advisories. Artwork in ntfy alerts moved to the **`ntfy-images` extra** and **`NTFY_IMAGES` now defaults to `False`**. Install it with `pip install "spotify_monitor[ntfy-images]"`, or let the **setup wizard offer to install it for you** when you choose ntfy. Python 3.10 and newer get the current Pillow 12 while **Python 3.9 keeps the last release that supports it**, and **Python 3.9 through 3.14 remain supported**. The published **Docker images already include artwork support**, so only `NTFY_IMAGES = True` is needed there. If artwork is enabled without Pillow, startup and `--doctor` name the exact install command for your install method and alerts keep arriving as text. **Upgrading users who relied on the old default should set `NTFY_IMAGES = True`** to keep cover art in ntfy notifications
...
- **IMPROVE:** **Verifiable release downloads** - Each release now attaches a **`SHA256SUMS.txt`** file next to the `zip` and `tar.gz` archives, and both archives carry a signed build provenance attestation. Confirm a download really came from this repository before unpacking it with `gh attestation verify spotify_monitor_<tag>.zip --repo misiektoja/spotify_monitor`
- **IMPROVE:** **Guarded releases** - **A release cannot be published until the full test suite passes**, on PyPI as well as Docker Hub, so an untested build never ships under the project's name. Release workflows pin every third-party action to a fixed commit and automated checks confirm the version you see in `--version`, the package metadata and these release notes always agree
- **IMPROVE:** **Refreshed Docker images** - **Both published images now run on Python 3.13 and Debian 13**, apply pending Debian security updates while they build and no longer ship `pip` in the runtime, so a fresh `docker pull` of `misiektoja/spotify-monitor` or `misiektoja/spotify-secrets-grabber` carries no known fixable high or critical vulnerabilities. How you run them is unchanged: the same command lines, the same `/data` and `/work` mounts and the same Compose files. Both images now **pin their Python base image by digest**, so a rebuilt upstream tag cannot swap the runtime underneath a build, and Dependabot refreshes that digest weekly while holding the Python major and minor version steady
- **IMPROVE:** **Automatically published secret grabber image** - The debug image on Docker Hub is no longer built by hand. It is **tagged with the extractor's own version**, such as `misiektoja/spotify-secrets-grabber:1.4`, published whenever the extractor or its container files change and **rebuilt weekly** so a tag you already pulled keeps picking up Debian and Chromium security updates. Every publish is gated on the full test suite, and `latest` always points at the newest build
- **IMPROVE:** **Continuous security scanning** - Dependencies, both container images and the source code are now scanned on every change and again weekly, so a vulnerability published after a release is still caught. Each release publishes an **SBOM** listing everything you actually install, and the project's security practices are scored publicly by **OpenSSF Scorecard**
- **IMPROVE:** **Security policy and support guidance** - A **security policy** explains how to report a vulnerability privately, [SUPPORT.md](https://github.com/misiektoja/spotify_monitor/blob/main/SUPPORT.md) shows where a question, a bug report and a vulnerability each belong, and new issue forms collect the version, install method and `--doctor` output up front. Contribution guidance, a code of conduct, a third-party licensing notice and a test suite guide were added for contributors, along with linting and pre-commit hooks that run on every change. The repository page also offers **Cite this repository** for a ready-made BibTeX or APA entry
...
- **BUGFIX:** **Webhook delivery respects `VERIFY_SSL`** - Discord and ntfy deliveries now use the same TLS verification setting as every other request instead of always verifying, so a **TLS-inspecting proxy** setup that works for monitoring no longer fails only for webhooks. Every delivery now takes one shared request path that also **refuses redirects**, so a webhook address that redirects can no longer hand your alert and its headers to another host
...
- **BUGFIX:** **Declarative configuration and private backups** - Configuration files now accept only documented settings and reject imports, calls or any other code, so a configuration file sitting in the working directory can no longer run. The rejected line and setting are named on startup and by `--doctor`. **Configuration files carried across upgrades keep working:** settings that an older version wrote and a newer one removed, such as `SECRET_CIPHER_DICT`, `SECRET_CIPHER_DICT_URL` and `TOTP_VER` from versions 2.3.1 through 2.9.2, are ignored with a note instead of stopping startup, while a misspelled setting is still reported. Generated backups keep owner-only permissions
- **BUGFIX:** **Delivery-aware monitoring alerts** - Alerts now reflect whether they actually reached you. A failed Friend Activity alert is retried on the next check, while scrobble outage, reminder and recovery notices stay pending across restarts until one configured channel succeeds
- **BUGFIX:** **No more false Last.fm outages** - Repeated plays of the same artist and track are now matched pairwise instead of greedily, so a listening session with repeats can no longer look like a scrobble outage
- **BUGFIX:** **Accurate dotenv and activity-file state** - `SIGHUP` clears secrets removed from the selected dotenv file and resets affected authentication caches. Activity flag files are written atomically, stale startup flags are validated and a failure now stops or visibly disables the integration instead of leaving stale state behind
- **BUGFIX:** **Safe local playback and durable deadlines** - Track IDs are validated before local Spotify playback and the Linux and Windows integrations no longer build shell commands. The anti-hang watchdog can no longer be disabled by a nested request and is sized so it stops interrupting legitimate slow requests
- **BUGFIX:** **Lossless Unicode config and leaner polling** - Generated configuration preserves supplementary Unicode characters such as emoji. **Both cookie and advanced client mode** now trust a cached access token until it expires instead of probing it before every poll, roughly halving Friend Activity requests to Spotify and reducing the chance of hitting a rate limit. A token Spotify rejects early is detected from the poll itself and refreshed on the next check
- **BUGFIX:** **Safer debug utilities and container output** - Token validation accepts only HTTPS Spotify API destinations without redirects. Both debug utilities return a nonzero exit status when requested work fails and the secret-grabber image runs non-root with configurable Compose UID and GID mapping for host-owned output. The secret grabber now explains how to install Playwright instead of failing on startup when it is missing
