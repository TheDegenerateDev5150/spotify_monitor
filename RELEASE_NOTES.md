...
- **IMPROVE:** **Safer Pillow floor** - The minimum **Pillow** version moved from 10.0 to **11.3**, so a fresh install no longer resolves to a years-old image library. **Python 3.9 through 3.14 remain supported** and the install process is unchanged. On Python 3.10 and newer, pip picks up the current Pillow automatically
