...
- **IMPROVE:** **Refreshed Docker images** - **Both published images now run on Python 3.13** from an actively maintained base and no longer ship `pip` in the runtime, so a fresh `docker pull` of `misiektoja/spotify-monitor` or `misiektoja/spotify-secrets-grabber` carries no known fixable high or critical vulnerabilities. The secret grabber image also applies pending Debian security updates while it builds. How you run them is unchanged: the same command lines, the same `/data` and `/work` mounts and the same Compose files
