...
- **IMPROVE:** **Refreshed Docker image** - The published image now runs on **Python 3.13** from an actively maintained base and no longer ships `pip` in the runtime, so a fresh `docker pull misiektoja/spotify-monitor:latest` carries no known fixable high or critical vulnerabilities. How you run it is unchanged: the same command line, the same `/data` mount and the same Compose file
- **IMPROVE:** **Continuous security scanning** - Dependencies, the container image and the source code are now scanned on every change and again weekly, so a vulnerability published after a release is still caught. Each release publishes an **SBOM** listing everything you actually install, and the project's security practices are scored publicly by **OpenSSF Scorecard**
